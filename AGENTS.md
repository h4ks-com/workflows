...
- With `N8N_API_KEY` set, the n8n provider (`src/workflows/n8n.py`) serves every active n8n workflow tagged `h4ks-workflows`, read through the n8n public API with a read-only key. Each needs an enabled Webhook node (its path is the executor URL), a Form Trigger node (usually disabled) whose title, description and fields define the form, and a sticky note with a fenced `h4ks-workflows` JSON block: `name`, `pricing`, `price`, `steps`, optional `position` and per-field `fields` overrides (`description`, `type`, `minimum`, `maximum`, `min_length`, `max_length`, `show_when`). A workflow that breaks these rules is skipped and logged. Without the key, the built-in provider serves the types in `builtin.py` with an `HttpExecutor` that posts to `N8N_URL/webhook/<webhook path>`.
