...
- Job types live in code in `jobtypes.py`. Each type declares its n8n webhook path; `N8N_URL` points at the n8n instance, and its executor URL is `N8N_URL/webhook/<webhook path>`. A type with `webhook=None` has no executor yet, is listed as unavailable and refuses quotes and orders. Adding a workflow means adding a params model and a `JobType` entry with its webhook path, nothing else.
