...
- `runs/`: job lifecycle and the executor callback contract `jobs.py`; queue worker (dispatch, timeout watchdog) `worker.py`; queue holds `holds.py`; filled-form links `drafts.py`; job status subscriptions `webhooks.py`; ETAs `eta.py`; in-process event bus `bus.py`
- `accounts/`: auth dependencies (session user, admin, admin token, service token, CSRF) `auth.py`; Logto login `login.py`; account, wallet and top-ups `account.py`; credits ledger `ledger.py`; Beans top-up poller `beans.py`
- `api/`: JSON API under `/api` `routes.py` with response models in `views.py`; SSE streams `stream.py`; MCP at `/mcp` `mcp.py`; client identity linking, drafts and subscriptions `clients.py`; filled-form links `drafts.py`; admin API `admin.py`
...
- An admin can send the whole Beans wallet of the service to the one account in `BEANS_PAYOUT_USER` from the admin page (`accounts/beans.py` `BeansPayout`). It takes no recipient or amount from the request, reads the balance at the moment of sending, never lets Beans create a wallet, and runs one payout at a time.
...
- A `JobType` carries its `Form`, a `PriceRule`, its steps and an optional `Executor`. A type without an executor is listed as unavailable and refuses quotes and submissions. The form is the single source for the web form, the JSON schema in the API and MCP, and strict validation; a field's `show_when` (equals, `not`, `one_of`, `not_one_of` or `empty` per field, checked the same way in `forms.py` and `app.js`) shows it and makes it required only while the conditions hold, and a hidden field is validated to `null`.
...
- Executor dispatch carries the `X-API-Key` header and a JSON body with `job_id`, `type`, `params`, `media`, `steps` (step names in order), `callback_url` and `callback_token`. Executors call back with the per-job bearer token; we store only its hash. Before dispatch, `runs/media.py` downloads every link field that accepts audio through ytdl `/v1/file` into the private `INPUTS_BUCKET`, puts a 12-hour presigned URL in its param and the title and duration under `media.<field>`, and notes both in the run log; a link it cannot download fails the job with ytdl's reason and refunds it.
...
- The only way to submit a job is the web form `POST /submit/<type>`, by a logged-in user. Clients such as bots and MCP agents make drafts for people to submit.
- A trusted client can `PUT /api/clients/subscription` with `url` and `signing_key` (upserted by `url`, removed with `DELETE /api/clients/subscription?url=...`) to get a signed raw event for every paid job when it is queued, starts, succeeds, fails or is cancelled. The body is `event`, `job_id`, `type`, `type_title`, `status`, `owner`, `title` (the result title), `run_url`, `result_urls`, and `error`, with header `X-Webhook-Signature: sha256=<hex>` where `hex` is `HMAC-SHA256(signing_key, json.dumps(payload, sort_keys=True))`. Delivery is best-effort: it retries connection errors, timeouts and 5xx three times with backoff, gives up on 4xx, and never affects the job.
- A queue hold (`runs/holds.py`, stored in the database) keeps the worker from starting new jobs, for a number of minutes or until released; the running job finishes, submissions keep queuing, queue ETAs add the time left on the hold, and the queue page shows it. Admins set it on the admin page or with `PUT`/`DELETE /api/admin/queue/hold`; the admin API also accepts `ADMIN_TOKEN` as a bearer token, for scripts.
- A draft (`runs/drafts.py`) is a filled form behind a short link `/d/<id>`, made with `POST /api/clients/drafts` or the `share_filled_form` MCP tool, so someone not logged in gets a form ready to submit. Only form fields are accepted, missing ones are left for the user, and the link opens the submit page filled in. A draft lasts 7 days and is deleted when a job is submitted from it, when it is opened after expiring, and by an hourly sweep.
...
- Job type text matches the existing forms: the description is one short plain sentence naming what the job makes ("Make a karaoke video from a song."), never its formats, options or how it works; a field's help text is one short sentence the user needs to fill it in.
