...
- `runs/`: job lifecycle and the executor callback contract `jobs.py`; queue worker (dispatch, timeout watchdog) `worker.py`; queue holds `holds.py`; filled-form links `drafts.py`; job status webhooks `webhooks.py`; ETAs `eta.py`; in-process event bus `bus.py`
...
- `api/`: JSON API under `/api` `routes.py` with response models in `views.py`; SSE streams `stream.py`; MCP at `/mcp` `mcp.py`; client job submission, identity linking and confirm pages `clients.py`; filled-form links `drafts.py`; admin API `admin.py`
